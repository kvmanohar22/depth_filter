  ...
  : ref_idx_(ref_idx)
{
  ...
  io_ = new RPGSyntheticForward(root_dir_);
  DLOG(INFO) << "Read " << io_->n_imgs() << " files";
  load_ref();
}

void DepthFilterTest::load_ref() {
  dynamic_cast<io::RPGSyntheticForward*>(io_)->read_gtdepth(ref_idx_, camera_, depth_img_ref_);
...
void DepthFilterTest::run() {
  float depth_mean = 16.0f, depth_min = 1.0f, depth_max = 50.0f;
