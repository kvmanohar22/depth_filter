...
public:
 ...
 ~FastDetector() =default;
  ...
  static void detect(cv::Mat& img, list<Corner*>& corners) {
    ...
    std::for_each(keypoints.begin(), keypoints.end(), [&](cv::KeyPoint &kpt) {
      ...
      Corner *corner = new df::Corner(pt);
      corners.push_back(corner);
    });
  }

  static void detect(ImgPyr& pyr, list<Corner*>& corners) {
    vector<cv::KeyPoint> keypoints;
    cv::Ptr<cv::FastFeatureDetector> detector = cv::FastFeatureDetector::create();
    detector->detect(pyr, keypoints);
    std::for_each(keypoints.begin(), keypoints.end(), [&](cv::KeyPoint &kpt) {
      Vector2d pt(kpt.pt.x, kpt.pt.y);
      Corner *corner = new df::Corner(pt);
      corners.push_back(corner);
